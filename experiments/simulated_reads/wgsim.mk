...
r0_wgsim_$1.fq.gz: $$(FA) $$(QTIP_EXPERIMENTS_HOME)/software/wgsim/wgsim
	$$(QTIP_EXPERIMENTS_HOME)/software/wgsim/wgsim -S $5 -1 $3 -2 $3 -N $4 $2 $$(@:%.fq.gz=%.fq) .tmp.$$@
	rm -f .tmp.$$@
	gzip $$(@:%.fq.gz=%.fq)
	$$(QTIP_EXPERIMENTS_HOME)/software/wgsim/wgsim > $$@.version 2>&1 || true
...
r1_wgsim_$1.fq.gz: $(FA) $$(QTIP_EXPERIMENTS_HOME)/software/wgsim/wgsim
	$$(QTIP_EXPERIMENTS_HOME)/software/wgsim/wgsim -S $7 -d $5 -s $6 -1 $3 -2 $3 -N $4 $2 .tmp_1.$$@.fq .tmp_2.$$@.fq
	gzip -c .tmp_1.$$@.fq > $$@
	rm -f .tmp_1.$$@.fq
	gzip -c .tmp_2.$$@.fq > $$(@:r1_%=r2_%)
	rm -f .tmp_2.$$@.fq
	$$(QTIP_EXPERIMENTS_HOME)/software/wgsim/wgsim 2>&1 > $$@.version || true

endef

$(QTIP_EXPERIMENTS_HOME)/software/wgsim/wgsim:
	$(MAKE) -C $(QTIP_EXPERIMENTS_HOME)/software/wgsim
