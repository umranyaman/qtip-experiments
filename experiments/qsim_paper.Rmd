...
various_sensitivities_table <- function(want_training, want_paired, want_local, want_readlen) {
  ...
  oatmp <- subset(oatmp, select=c(sensitivity,
                                  overall_test_auc_diff_pct_round,
                                  overall_test_mse_diff_pct_round))
  levels(oatmp$sensitivity)[grepl('^vs', levels(oatmp$sensitivity))] <- "--very-sensitive"
  levels(oatmp$sensitivity)[grepl('^s', levels(oatmp$sensitivity))] <- "--sensitive"
  levels(oatmp$sensitivity)[grepl('^f', levels(oatmp$sensitivity))] <- "--fast"
  levels(oatmp$sensitivity)[grepl('^vf', levels(oatmp$sensitivity))] <- "--very-fast"
...
# Write to PDF
pdf(file="various_genomes_cid_test.pdf")
grid_arrange_shared_legend(p_vg_u100ei,
                           p_vg_u250ei,
                           p_vg_p100ei,
                           p_vg_p250ei)
...
# ERR050083 & ERR050082 (columns)
# Unpaired & paired (rows)
# Different aligners (chunks of rows)

real_data_accuracy_table <- function(dataset) {
  real %>% filter(data==dataset) %>%
           select(data, paired, aligner, loose, RCA) %>%
           arrange(paired, aligner, loose)
}
kable(real_data_accuracy_table("ERR050082"), format="markdown")
kable(real_data_accuracy_table("ERR050083"), format="markdown")
kable(real_data_accuracy_table("ERR050082"), format="latex")
kable(real_data_accuracy_table("ERR050083"), format="latex")
```
