...
trials_plot <- function(m_filt, want_cid, xlab) {
  ...
  mx <- max(abs(mtmp$y))
  ggplot(mtmp, aes(x=x, y=y, color=factor(trial_no))) +
...
subsample_trials_plot <- function(m_filt, want_cid) {
  f01_te <- trials_plot(m_filt %>% filter(subsampling_fraction == 0.01 & !training), want_cid, "0.01 test")
  f01_tr <- trials_plot(m_filt %>% filter(subsampling_fraction == 0.01 &  training), want_cid, "0.01 training")
  f03_te <- trials_plot(m_filt %>% filter(subsampling_fraction == 0.03 & !training), want_cid, "0.03 test")
  f03_tr <- trials_plot(m_filt %>% filter(subsampling_fraction == 0.03 &  training), want_cid, "0.03 training")
  f05_te <- trials_plot(m_filt %>% filter(subsampling_fraction == 0.05 & !training), want_cid, "0.05 test")
  f05_tr <- trials_plot(m_filt %>% filter(subsampling_fraction == 0.05 &  training), want_cid, "0.05 training")
  grid_arrange_shared_legend(f01_te, f01_tr, f03_te, f03_tr, f05_te, f05_tr)
}
```
...
oa <- oa[oa$subsampling_fraction == 0.03,]  # for now
...
various_lengths_table <- function(want_training, want_paired, want_local) {
  ...
  oatmp$RCA <- paste0(prettyNum(oatmp$RCA, digits=4, format='fg'), '%')
  oatmp$RCA <- ifelse(oatmp$RCA >= 0.0, paste0('+', oatmp$RCA), oatmp$RCA)
  oatmp$RCE <- paste0(prettyNum(oatmp$RCE, digits=4, format='fg'), '%')
  oatmp$RCE <- ifelse(oatmp$RCE >= 0.0, paste0('+', oatmp$RCE), oatmp$RCE)
  xtable(oatmp)
}

various_lengths_table(F, F, F)
various_lengths_table(F, F, T)
various_lengths_table(F, T, F)
various_lengths_table(F, T, T)
...
various_sensitivities_table <- function(want_training, want_paired, want_local, want_readlen) {
  ...
  oatmp <- subset(oatmp, select=c(sensitivity,
                                  overall_test_auc_diff_pct_round,
                                  overall_test_mse_diff_pct_round))
  levels(oatmp$sensitivity)[levels(oatmp$sensitivity)=="vs"] <- "--very-sensitive"
  levels(oatmp$sensitivity)[levels(oatmp$sensitivity)=="s"] <- "--sensitive"
  levels(oatmp$sensitivity)[levels(oatmp$sensitivity)=="f"] <- "--fast"
  levels(oatmp$sensitivity)[levels(oatmp$sensitivity)=="vf"] <- "--very-fast"
...
various_sensitivities_table(F, F, F, 100)
various_sensitivities_table(F, F, T, 100)
various_sensitivities_table(F, T, F, 100)
various_sensitivities_table(F, T, T, 100)
...
various_aligners_table(F, F, 100)
various_aligners_table(F, T, 100)
various_aligners_table(F, F, 250)
various_aligners_table(F, T, 250)
```
...
Zooming in on SNAP 250 x 250 CID unpaired:

```{r various_lengths_plot_snap_250_unp}
subsample_trials_plot(m %>% filter(name == 'various_aligners' & !paired & readlen == 250 & aligner == 'snap'), T)
```

And paired:

```{r various_lengths_plot_snap_250_pe}
subsample_trials_plot(m %>% filter(name == 'various_aligners' & paired & readlen == 250 & aligner == 'snap'), T)
```
...
Zooming in on BWA-MEM 100 x 100 human CID:

```{r various_genomes_plot_bwa_100_pe_hg}
subsample_trials_plot(m %>% filter(name == 'various_genomes' & species == 'hg' & paired & readlen == 100 & aligner == 'bwamem'), T)
```

Now mouse:

```{r various_genomes_plot_bwa_100_pe_mm}
subsample_trials_plot(m %>% filter(name == 'various_genomes' & species == 'mm' & paired & readlen == 100 & aligner == 'bwamem'), T)
```

Now corn:

```{r various_genomes_plot_bwa_100_pe_zm}
subsample_trials_plot(m %>% filter(name == 'various_genomes' & species == 'zm' & paired & readlen == 100 & aligner == 'bwamem'), T)
```

And 250 x 250 human:

```{r various_genomes_plot_bwa_250_pe_hg}
subsample_trials_plot(m %>% filter(name == 'various_genomes' & species == 'hg' & paired & readlen == 250 & aligner == 'bwamem'), T)
```

Now mouse:

```{r various_genomes_plot_bwa_250_pe_mm}
subsample_trials_plot(m %>% filter(name == 'various_genomes' & species == 'mm' & paired & readlen == 250 & aligner == 'bwamem'), T)
```

Now corn:

```{r various_genomes_plot_bwa_250_pe_zm}
