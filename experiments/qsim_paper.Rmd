...
oa <- m[m$trial_no == 1,]  # for now

various_lengths_table <- function(want_training, want_paired, want_local) {
  oatmp <- oa %>% filter(training == want_training & name == 'ill_various_length' &
                           paired == want_paired & local == want_local)
  oatmp <- subset(oatmp, select=c(readlen,
                                  overall_test_auc_diff_pct_round,
                                  overall_test_mse_diff_pct_round))
  colnames(oatmp) <- c('Read length', 'RCA', 'RCE')
  oatmp$RCA <- paste0(formatC(oatmp$RCA, digits=2, format='f', flag='#'), '%')
  oatmp$RCA <- ifelse(oatmp$RCA >= 0.0, paste0('+', oatmp$RCA), oatmp$RCA)
  oatmp$RCE <- paste0(formatC(oatmp$RCE, digits=2, format='f', flag='#'), '%')
  oatmp$RCE <- ifelse(oatmp$RCE >= 0.0, paste0('+', oatmp$RCE), oatmp$RCE)
  xtable(oatmp)
}
...
write("% cut the multicolumn/midrule line pairs and paste them down just before the results lines", file=fn, append=T)
write("% get rid of the bottomrule, end tabular, begin tabular, toprule, header lines separating the chunks of results", file=fn, append=T)
xt <- various_lengths_table(F, F, F)  # unpaired, end-to-end
write("\\multicolumn{3}{c}{Unpaired, end-to-end} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
xt <- various_lengths_table(F, F, T)  # unpaired, local
write("\\multicolumn{3}{c}{Unpaired, local} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
xt <- various_lengths_table(F, T, F)  # paired, end-to-end
write("\\multicolumn{3}{c}{Paired, end-to-end} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
xt <- various_lengths_table(F, T, T)  # paired, local
write("\\multicolumn{3}{c}{Paired, local} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
write("\\caption{Relative change in area under CID (RCA) and relative change in sum of squared error (RCE) for simulations spanning various read lengths.  Reads are either unpaired and paired-end and Bowtie 2 is run in either end-to-end or local alignment mode as indicated.}", file=fn, append=T)
write("\\label{length_tab}", file=fn, append=T)
write("\\end{table}", file=fn, append=T)

various_sensitivities_table <- function(want_training, want_paired, want_local, want_readlen) {
  oatmp <- oa %>% filter(training == want_training & name == 'various_sensitivities' &
                           paired == want_paired & local == want_local &
                           readlen == want_readlen)
  oatmp <- subset(oatmp, select=c(sensitivity,
                                  overall_test_auc_diff_pct_round,
                                  overall_test_mse_diff_pct_round))
  levels(oatmp$sensitivity)[grepl('^vs', levels(oatmp$sensitivity))] <- "\\texttt{--very-sensitive}"
  levels(oatmp$sensitivity)[grepl('^s', levels(oatmp$sensitivity))] <- "\\texttt{--sensitive}"
  levels(oatmp$sensitivity)[grepl('^f', levels(oatmp$sensitivity))] <- "\\texttt{--fast}"
  levels(oatmp$sensitivity)[grepl('^vf', levels(oatmp$sensitivity))] <- "\\texttt{--very-fast}"
  colnames(oatmp) <- c('Sensitivity', 'RCA', 'RCE')
  oatmp$RCA <- paste0(formatC(oatmp$RCA, digits=2, format='f', flag='#'), '%')
  oatmp$RCA <- ifelse(oatmp$RCA >= 0.0, paste0('+', oatmp$RCA), oatmp$RCA)
  oatmp$RCE <- paste0(formatC(oatmp$RCE, digits=2, format='f', flag='#'), '%')
  oatmp$RCE <- ifelse(oatmp$RCE >= 0.0, paste0('+', oatmp$RCE), oatmp$RCE)
...
write("% cut the multicolumn/midrule line pairs and paste them down just before the results lines", file=fn, append=T)
write("% get rid of the bottomrule, end tabular, begin tabular, toprule, header lines separating the chunks of results", file=fn, append=T)
xt <- various_sensitivities_table(F, F, F, 100)
write("\\multicolumn{3}{c}{Unpaired, end-to-end} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
xt <- various_sensitivities_table(F, F, T, 100)
write("\\multicolumn{3}{c}{Unpaired, local} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
xt <- various_sensitivities_table(F, T, F, 100)
write("\\multicolumn{3}{c}{Paired, end-to-end} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
xt <- various_sensitivities_table(F, T, T, 100)
write("\\multicolumn{3}{c}{Paired, local} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
write("\\caption{Relative change in area under CID (RCA) and relative change in sum of squared error (RCE) for various sensitivity levels for Bowtie 2.  Experiments use 100 nt unpaired or paired-end as indicated.}", file=fn, append=T)
write("\\label{sens_tab}", file=fn, append=T)
write("\\end{table}", file=fn, append=T)

various_aligners_table <- function(want_training, want_paired, want_readlen, ...) {
  oatmp <- oa %>% filter(training == want_training & name == 'various_aligners' &
                           paired == want_paired & readlen == want_readlen)
  oatmp <- subset(oatmp, select=c(aligner,
                                  local,
                                  overall_test_auc_diff_pct_round,
                                  overall_test_mse_diff_pct_round))
  levels(oatmp$aligner)[levels(oatmp$aligner)=="bt2"] <- "Bowtie 2"
  levels(oatmp$aligner)[levels(oatmp$aligner)=="bwamem"] <- "BWA-MEM"
  levels(oatmp$aligner)[levels(oatmp$aligner)=="snap"] <- "SNAP"
  oatmp$local <- ifelse(oatmp$local, 'local', 'end-to-end')
  colnames(oatmp) <- c('Aligner', 'Local?', 'RCA', 'RCE')
  oatmp$RCA <- paste0(formatC(oatmp$RCA, digits=2, format='f', flag='#'), '%')
  oatmp$RCA <- ifelse(oatmp$RCA >= 0.0, paste0('+', oatmp$RCA), oatmp$RCA)
  oatmp$RCE <- paste0(formatC(oatmp$RCE, digits=2, format='f', flag='#'), '%')
  oatmp$RCE <- ifelse(oatmp$RCE >= 0.0, paste0('+', oatmp$RCE), oatmp$RCE)
...
fn <- 'various_aligners.tex_snippet'
write("\\begin{table}", file=fn)
write("\\centering", file=fn, append=T)
write("% cut the multicolumn/midrule line pairs and paste them down just before the results lines", file=fn, append=T)
write("% get rid of the bottomrule, end tabular, begin tabular, toprule, header lines separating the chunks of results", file=fn, append=T)
xt <- various_aligners_table(F, F, 100)
write("\\multicolumn{4}{c}{Unpaired, 100 nt} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
xt <- various_aligners_table(F, T, 100)
write("\\multicolumn{4}{c}{Paired, 100 x 100 nt} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
xt <- various_aligners_table(F, F, 250)
write("\\multicolumn{4}{c}{Unpaired, 250 nt} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
xt <- various_aligners_table(F, T, 250)
write("\\multicolumn{4}{c}{Paired, 250 x 250 nt} \\\\", file=fn, append=T)
write("  \\midrule", file=fn, append=T)
write(xt, file=fn, append=T)
write("\\caption{Relative change in area under CID (RCA) and relative change in sum of squared error (RCE) for various read aligners.  Experiments use 100 nt or 250 nt reads and unpaired or paired-end reads as indicated.}", file=fn, append=T)
write("\\label{aligners_tab}", file=fn, append=T)
write("\\end{table}", file=fn, append=T)
```
...
various_genomes_table <- function(m, want_training, ...) {
  ...
  tabular(((Heading("Unpaired") * (!paired) + Heading("Paired") * paired) * (Heading("100 nt") * (readlen == 100) + Heading("250 nt") * (readlen == 250)) * Heading() * aligner) ~ (Format(digits=2) * Heading() * species * (RCE + RCA) * (mean + sd)), data=mtmp)
}
...
write("\\caption{Relative change in area under CID (RCA) and relative change in sum of squared error (RCE) for various aligners and reference genomes.  Values reported are the mean and standard deviation for RCA and RCE over 5 random trials, with a distinct pseudo-random seed used in each trial.  Mean RCE and RCA are expressed in percent.  The experiments used 100 nt or 250 nt reads and unpaired or paired-end reads as indicated by the row labels.}", file=fn, append=T)
...
various_simulators_table <- function(m, want_training, ...) {
  ...
  tabular((Heading("Unpaired") * (!paired) + Heading("Paired") * paired) *
          ...
          Heading() * simulator ~ ((RCE + RCA) * (mean + sd)), data=mtmp)
}
...
write("\\caption{Relative change in area under CID (RCA) and relative change in sum of squared error (RCE) for samples simulated by various simulators.  Bowtie 2 end-to-end alignment was used in all cases. Values reported are the mean and standard deviation for RCA and RCE over 5 random trials, with a distinct pseudo-random seed used in each trial.  Mean RCE and RCA are expressed in percent.  The experiments used 100 nt or 250 nt reads and unpaired or paired-end reads as indicated by the row labels.}", file=fn, append=T)
...
real_data_accuracy_table <- function() {
  ...
  tabular(((Heading("Unpaired") * (!paired) + Heading("Paired") * paired) *
            Heading() * aligner) ~
           (Format(digits=2) * Heading() * data * (RCE + RCA) * (mean + sd)),
          data=realtmp)
}
...
write("\\caption{Relative change in area under CID (RCA) and relative change in sum of squared error (RCE) for various aligners on the public ERR050082 and ERR050083 DNA sequening samples.  Values reported are the mean and standard deviation for RCA and RCE over 5 random trials, with a distinct pseudo-random seed used in each trial.  Mean RCE and RCA are expressed in percent.  Both samples are paired-end, but in some experiments (``Unpaired'') we supply only the first end to the aligner.}", file=fn, append=T)
...
real_data_perf_table <- function(dataset) {
  perf %>% filter(data==dataset) %>%
           select(data, paired, aligner, align_time, overall_time,
                  pct_increase_a_to_o,
                  peak_wrapper, peak_children, pct_increase_peak) %>%
           arrange(paired, aligner)
}
kable(real_data_perf_table("ERR050082"), format="markdown")
kable(real_data_perf_table("ERR050083"), format="markdown")
kable(real_data_perf_table("ERR050082"), format="latex")
kable(real_data_perf_table("ERR050083"), format="latex")
```
