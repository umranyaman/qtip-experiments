...
various_genomes_table <- function(m, want_training, ...) {
  ...
  tabular(((Heading("Unpaired") * (!paired) + Heading("Paired") * paired) * (Heading("100 nt") * (readlen == 100) + Heading("250 nt") * (readlen == 250)) * Heading() * aligner) ~ (Format(digits=2) * Heading() * species * (RCA + RCE) * (mean + sd)), data=mtmp)
}
