        ...
        for ln in fh:
            if pred_re.match(ln):
                ...
                if len(ln.rstrip()) == 0:
                    ...
                    # subsampling png
                    subsampling_png_fn = '%s_%s_subsampling_series.png' % (name, target)
                    subsampling_png_full = os.path.join(target_full, subsampling_png_fn)
                    subsampling_png_out = os.path.join('summary/subsampling_plots')
                    mkdir_quiet(subsampling_png_out)
                    shutil.copy(subsampling_png_full, subsampling_png_out)
