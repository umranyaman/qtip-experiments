...
_mason_orig_beg = re.compile('BEGIN_POS=([0-9]*)')
_mason_samp_seq = re.compile('SAMPLE_SEQUENCE=([A-Za-z.]*)')
...
def parse_mason(nm):
    ...
    assert be is not None, nm

    seq = _mason_samp_seq.search(nm)
    assert seq is not None, nm
    ...
    return left+1, right, refid, strand == 'F'
